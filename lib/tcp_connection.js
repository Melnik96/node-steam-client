...
TCPConnection.prototype.connect = function(options, callback) {
	if (options.httpProxy) {
		var self = this;
		var url = new URL(options.httpProxy);
		url.method = 'CONNECT';
		...
		var url = new URL(options.socksProxy);
		...
		var sock_options = {
			...
			port: options.port
                ...
                console.log('proxy options', sock_options, options);
		
		Socks.createConnection(sock_options, (err, socket, info) => {
		    if (err)
			console.log(err);
		    else {
			console.log("Socks Proxy Connected");
